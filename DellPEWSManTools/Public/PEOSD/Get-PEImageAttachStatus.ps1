<#
Get-PEImageAttachStatus.ps1 - Gets the status of image or driver pack attach.
...
function Get-PEImageAttachStatus
{
    [CmdletBinding()]
    param (
        [Parameter(Mandatory)]
    ...
    Process
    {
        Invoke-CimMethod -InputObject $instance -CimSession $iDRACSession -MethodName GetAttachStatus
    }
}
